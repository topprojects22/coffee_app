...
class RetryInterceptor extends Interceptor {
  ...
  Future<void> onError(
    ...
    final bool shouldRetry =
        extra.retries > 0 && await options.retryEvaluator(err);
    if (!shouldRetry) {
    ...
    await dio
        ...
        .then((Response value) => handler.resolve(value),
...
class RetryOptions {
  ...
  static FutureOr<bool> defaultRetryEvaluator(DioException error) {
    final bool cancelError = error.type != DioExceptionType.cancel;
    final bool responseError = error.type != DioExceptionType.badResponse;
    final bool shouldRetry = cancelError && responseError;
    return shouldRetry;
  }
